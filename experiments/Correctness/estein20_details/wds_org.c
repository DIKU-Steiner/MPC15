...
static FILE *debugf;// = fopen("file.txt", "w");
...
void output_tree(int* A)
{
	int i, j;
	fprintf(debugf,"topology-describing vector:");
	for (i = 1; i <= NUMSITES-3; i++) fprintf(debugf," %d", A[i]);
	fprintf(debugf,"\nterminal point coords\n");
	for (i = 1; i <= NUMSITES; i++) {
		for (j = 0; j < DIMENSION; j++) fprintf(debugf," %20.20g", XX[i][j]);
		fprintf(debugf,"\n");
	}
	...
	fprintf(debugf,"edges\n");
	for (i = 1; i <= 2*NUMSITES-3; i++) fprintf(debugf,"%d-%d;", edge[i][0], edge[i][1]);
	fprintf(debugf,"\n");
	fflush(debugf);
}
...
{ /* Inputs NUMSITES, DIMENSION, sites; outputs succesive best Steiner
   * trees found. Best tree's topology-vector is stored in BESTVEC. */
	...
	REAL tolFactor = 0.0001;
	for(i=1;i<argc-1;i++){
		if(0==strcmp("-tol", argv[i])){
			tolFactor = atof(argv[i+1]);
			printf("Setting tolerance factor to %.8f\n",tolFactor);
		}
	}
...
ITER: /* .. and optimize it until either obviously bad or small error figure happens */
			...
			if(debugmode){
				...
				int t;
				for(t = 1; t<=k; t++)
					fprintf(debugf, "%d ", A[t]);
				fprintf(debugf, "\n");
				fprintf(debugf, "LENGTH: %.4f\n",q);
				fprintf(debugf, "ERROR:  %.4f\n",r);
				fprintf(debugf, "STUB:   %.4f\n",STUB);
				output_tree(A);

			}


			if ( debugmode && (q - r >= STUB)) { 
				fprintf(debugf,"REJECTING\n");
			}
			if ( q - r < STUB) { 
				if (r > 50*tolFactor*q) { optimize(tolFactor*r/NUMSITES); goto ITER; }
				if (k >= NUMSITES-3) {
					do { optimize(tolFactor*r/NUMSITES); q = length(); r = error(); 
						/* printf("%20.20g %20.20g\n", q, r); */
					} while (r > q*tolFactor);
		...
		while (nc <= 0) {
			...
			if (k <= 0) { printf("done\n"); fflush(stdout); 
				...
				time_t t = clock() - start_t;
				printf ("Took %lld clocks = %f seconds.\n",t,((double)t)/CLOCKS_PER_SEC);
				exit(0); }
			nc = STACK[m]; m--;
		}
