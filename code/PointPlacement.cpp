...
PointPlacement::PointPlacement(vector< geomNd::Point > sites):
    D(sites[0].dimensions()),
    N(sites.size())
    //points(N+N-2, geomNd::Point(D)),
    //adj(N-2, vector<int>(3, 0)),
    //EL(N-2, vector<double>(3, 0)),
    //B(N, vector<double>(3, 0)),
    //C(N, vector<double>(D, 0)),
    //eqnstack(N),
    //leafQ(N),
    //val(N)
{
...
double PointPlacement::getLowerbound(const double tol)
{
	return getLength(tol)-error();
}
double PointPlacement::getLength(const double tol)
{
    double q,r;
    do{
        q = length();
...
double PointPlacement::lengthDebug()
{
    int i2,j,m;
    int n0,n1,n2;
    double leng = 0.0, t, r;
...
void PointPlacement::optimize(const double tol)
{
    int m,j,i2, i;
...
ostream& operator<<(ostream& out, const PointPlacement& p)
{
    out<<"PointPlacement"<<endl;
    out<<"> adj:"<<endl;
    for(int i=0;i<3;i++){
